...
export function parseCSVData(csvText: string): Omit<BloodPressureReading, 'id'>[] {
  ...
  const lines = cleanText
    ...
    .filter((line) => !line.trim().toLowerCase().startsWith('sep=') && !line.trim().startsWith('#'));
  ...
  let ppConfirmedIdx = headers.findIndex(
    (h) => h.includes('pulse_pressure_confirmed') || h.includes('presion_pulso_confirmada')
  );
  let medicationContextIdx = headers.findIndex(
    (h) => h.includes('medication_context') || h.includes('contexto_medicacion')
  );
  ...
  for (let i = startLineIdx; i < lines.length; i++) {
    ...
    const rawMedicationContext = medicationContextIdx !== -1
      ? (cols[medicationContextIdx] || '').trim().toLowerCase()
      : '';
