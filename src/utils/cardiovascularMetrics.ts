...
export function calculateEstimatedMeanArterialPressure(
  systolic: number,
  diastolic: number
): number {
...
export function calculateCardiovascularMetrics(
  ...
  return {
    pulsePressure: {
      average: pulsePressureValues.length === 0
        ? 0
        : Math.round(
          pulsePressureValues.reduce((sum, value) => sum + value, 0) /
          pulsePressureValues.length
        ),
    },
    estimatedMap: {
      average: mapValues.length === 0
        ? 0
        : Math.round(mapValues.reduce((sum, value) => sum + value, 0) / mapValues.length),
    ...
    pressureLoad: {
      ...
      dayCount,
      hasSufficientData:
        totalSessions >= MIN_HOME_BP_LOAD_SESSIONS && dayCount >= MIN_HOME_BP_LOAD_DAYS,
    },
  };
}
